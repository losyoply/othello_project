...
public:
    ...
    if(board[4][7]==2) w_total +=6;
    if(board[5][7]==2) w_total +=8;
    ////points around 金角 is bad if you didnt get 金角 yet
...
int minimax(int depth, OthelloBoard& board, int alpha, int beta)
{
    ...
    {
        return board.heuristic(); //board對AI有利程度
    }

    if(depth%2==1) //第一(三五七)層是AI，選對自己最有利的board (max)
    {
        int max_eval = -9999999; //有利程度最初設最小，最終是所有child有利值的最大值
    ...
    {
        int min_eval = 9999999; //偶數層(敵人) 選最不有利於AI的(min)
        std::vector<Point> child = board.get_valid_spots();
        std::vector<Point>::iterator it;
        for(it = child.begin();it!=child.end();it++)
        {
...
void write_valid_spot(std::ofstream& fout)
{
    ...
    Point best_point;
    int ans = -999999999; //預設最小，因為要找最大 (第一層AI)
    std::vector<Point>::iterator it;
    ...
    for(it = next_valid_spots.begin();it!=next_valid_spots.end();it++)
    {
        ...
        else m = minimax(2, next_board, alpha, beta);//這個board可得最佳解

        if(m>ans) //最佳解>ans ， 取代 ，並把best_point設成此 //alpha pruning ?????????s
        {
            ans = m;
            best_point = *it;
            alpha = m;
